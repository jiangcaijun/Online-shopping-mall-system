...
$(document).ready(function(){
	...
	$.getJSON("/user",{action:"editCom",eid:GetQueryString("id")},function(data){
...
function saveMyComment(){
	...
	var text = $("#myText").val();
	if(text!=null&&text!=""){
		$.getJSON("/user",{action:"editComment",star:star,text:text,exchangeId:GetQueryString("id"),date:new Date()},function(data){
			if(data.message=="true"){
				alert("评价成功");
				location.href="/customer/orderList.jsp";
			}
		});
	}else{
		alert("评价内容不能为空");
